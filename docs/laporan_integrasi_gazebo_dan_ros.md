...
> **⚠️ Bagian ini diisi oleh praktikan dengan menempelkan screenshot/output percobaan.**

### 5.1 Hasil Percobaan 1: Spawn URDF

**a. Tampilan Gazebo setelah spawn URDF:**

> _[Tempelkan screenshot Gazebo yang menampilkan robot setelah di-spawn]_

**b. Output terminal saat proses spawn:**

> _[Tempelkan screenshot/teks output terminal]_
>
> ```
> [Tempelkan output di sini]
> ```

**c. Verifikasi melalui topic list:**

> _[Tempelkan output `ros2 topic list`]_

---

### 5.2 Hasil Percobaan 2: ROS2 Integration Via Bridge

**a. Output terminal saat menjalankan bridge:**

> _[Tempelkan screenshot/teks output]_
>
> ```
> [Tempelkan output di sini]
> ```

**b. Daftar topik ROS2 dan Gazebo setelah bridge aktif:**

> _[Tempelkan output `ros2 topic list` dan `gz topic -l`]_

**c. Output `ros2 topic echo /cmd_vel` saat mengirim perintah:**
...
**e. Tampilan robot bergerak di Gazebo setelah menerima perintah `/cmd_vel`:**

> _[Tempelkan screenshot Gazebo]_
>
> **Sebelum:**
> _[Screenshot]_
>
...
**f. Output `ros2 topic echo /scan` (jika ada sensor LiDAR):**
...
### 5.3 Hasil Percobaan 3: ROS2 Interoperability

**a. Deteksi DDS yang aktif:**

> _[Tempelkan output `ros2 doctor --report`]_
>
> ```
> [Tempelkan output di sini]
> ```

**b. Hasil uji interoperabilitas Fast DDS ↔ Cyclone DDS:**

> _[Tempelkan output dari kedua terminal — publisher dan subscriber]_
>
> ```
> Publisher (Fast DDS):
> [Tempelkan output di sini]
...
**c. Hasil uji cross-domain (Domain 0 ↔ Domain 42):**

> _[Tempelkan output dari kedua terminal]_
>
> ```
> Domain 0 (Publisher):
> [Tempelkan output di sini]
>
> Domain 42 (Subscriber):
> [Tempelkan output di sini]
> ```

**d. Hasil uji ROS1-ROS2 Bridge (jika dilakukan):**
...
### 5.4 Hasil Percobaan 4: ROS2 Integration Template

**a. Struktur direktori template:**

> _[Tempelkan output `tree my_gz_robot/` atau screenshot file explorer]_
>
> ```
> [Tempelkan output di sini]
> ```

**b. Output build dengan colcon:**

> _[Tempelkan screenshot/teks output `colcon build`]_
>
> ```
> [Tempelkan output di sini]
> ```

**c. Tampilan Gazebo saat menjalankan launch file template:**

> _[Tempelkan screenshot Gazebo]_

**d. Daftar topik setelah seluruh launch file aktif:**

> _[Tempelkan output `ros2 topic list` dan `ros2 topic info -v` untuk topik penting]_
>
> ```
> [Tempelkan output di sini]
> ```

**e. Hasil modifikasi template (sesuaikan dengan modifikasi yang dilakukan):**

> _[Tempelkan screenshot yang menunjukkan perubahan — misalnya model robot baru, topik baru, atau world yang berbeda]_

**f. Output saat mengendalikan robot via teleop:**

> _[Tempelkan screenshot terminal teleop dan tampilan Gazebo]_
>
> ```
> [Tempelkan output di sini]
> ```
...
> **⚠️ Bagian ini diisi oleh praktikan berdasarkan hasil percobaan yang diperoleh.**

### 6.1 Analisis Percobaan 1: Spawn URDF

> **Panduan pembahasan:**
>
> - Apakah model URDF berhasil di-spawn dengan benar? Jika tidak, apa penyebabnya?
> - Apakah posisi dan orientasi robot sesuai dengan parameter yang diberikan?
> - Apakah terdapat perbedaan antara tampilan visual di URDF dan di Gazebo? Mengapa?
> - Apa fungsi plugin Gazebo pada URDF (misalnya `<gz:sensor>`, `<gz:joint>`)?
> - Bandingkan proses spawn manual (service call) vs melalui launch file.

> _[Tulis analisis di sini...]_

### 6.2 Analisis Percobaan 2: ROS2 Integration Via Bridge

> **Panduan pembahasan:**
>
> - Mengapa bridge diperlukan antara ROS2 dan Gazebo? (Jawab terkait perbedaan middleware)
> - Apa perbedaan arah bridge `ROS_TO_GZ` dan `GZ_TO_ROS`?
> - Bandingkan penggunaan bridge via CLI vs file konfigurasi YAML. Mana yang lebih baik untuk proyek besar?
> - Analisis data yang muncul di `/odom` — apakah nilainya wajar?
> - Analisis data `/scan` — apakah terdapat noise atau artefak?
> - Apa yang terjadi jika tipe data ROS2 dan Gazebo tidak sesuai?

> _[Tulis analisis di sini...]_

### 6.3 Analisis Percobaan 3: ROS2 Interoperability

> **Panduan pembahasan:**
>
> - Apakah pesan berhasil diterima saat menggunakan DDS berbeda? Mengapa?
> - Jelaskan peran DDS RMW dalam ROS2.
> - Mengapa pesan tidak diterima saat menggunakan domain yang berbeda? Apa fungsi `ROS_DOMAIN_ID`?
> - Dalam skenario apa interoperabilitas DDS menjadi penting?
> - Jika menguji ROS1-ROS2 bridge: apa keterbatasan yang ditemukan?

> _[Tulis analisis di sini...]_

### 6.4 Analisis Percobaan 4: ROS2 Integration Template

> **Panduan pembahasan:**
>
> - Apa keuntungan menggunakan template dibandingkan membangun dari nol?
> - Bagaimana hubungan antara file launch, file konfigurasi, dan URDF dalam template?
> - Analisis modifikasi yang dilakukan — apakah berhasil? Apa tantangannya?
> - Bagaimana template ini bisa dikembangkan untuk proyek yang lebih kompleks (misalnya navigasi autonom, SLAM, manipulasi)?
> - Bandingkan template ini dengan framework serupa (misalnya ros2_control, Nav2).
...
### Lampiran 1: File URDF yang Digunakan

```xml
<!-- Tempelkan isi file URDF lengkap di sini -->
```

### Lampiran 2: File Konfigurasi Bridge (bridge.yaml)

```yaml
# Tempelkan isi file bridge.yaml di sini
```

### Lampiran 3: File Launch

```python
# Tempelkan isi file launch Python di sini
```

### Lampiran 4: Output Lengkap Terminal

> _[Tempelkan output terminal lengkap yang tidak muat di bagian hasil percobaan]_

---

## **PETUNJUK PENGISIAN UNTUK PRAKTIKAN**

| Bagian                      | Yang Harus Dilakukan                                                                                                                            |
| --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| **Halaman Judul**           | Isi sesuai data diri                                                                                                                            |
| **Hasil Percobaan (Bab 5)** | Tempelkan **screenshot** dan **copy-paste teks output** terminal pada setiap bagian yang ditandai `>[Tempelkan ...]`                            |
| **Analisis (Bab 6)**        | Tulis paragraf analisis sendiri. Gunakan _panduan pembahasan_ sebagai acuan, tapi jangan hanya menjawab seperti Q&A — tulis dalam bentuk narasi |
| **Kesimpulan (Bab 7)**      | Rangkum **secara singkat dan tegas** apakah tujuan tiap percobaan tercapai atau tidak                                                           |
| **Lampiran (Bab 9)**        | Tempelkan seluruh file konfigurasi/kode yang digunakan selama praktikum                                                                         |
| **Daftar Pustaka**          | Isi tanggal akses dan tambahkan referensi lain jika ada                                                                                         |
